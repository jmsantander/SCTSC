...
class FanController(DeviceController):
    ...
    def __init__(self, device, config):
        ...
        self._voltage_sig = RandomSignal(hw_state['voltage'], 2 if add_noise else 0)
        self._current_sig = RandomSignal(hw_state['current'], 2 if add_noise else 0)
    ...
    def execute_command(self, command):
        ...
        if cmd == "open_connection":
            ...
            update = self.write_update('connected', int(self._isconnected))
            ...
            update = self.write_update('voltage', voltage, 'V')
        elif cmd == "read_current":
            if (self._is_on):
                current = self._current_sig.read() 
            else:
                current = self._zero_sig.read()
            update = self.write_update('current', current, 'A')
        else:
