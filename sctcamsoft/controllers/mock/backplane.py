...
class BackplaneController(DeviceController):
    ...
    def __init__(self, device, config):
        ...
        # ALABAMA PI TRIAL. HAS BEEN TESTED AND USE AS REFERENCE FOR TOMORROW.
        ip_address2 = '10.116.24.217'
        mypassword = '0'
        pi_path = '/home/pi/Desktop/BP_SPI_interface'
        pi_program = 'bp_test_pi'
        # # Initialize record of which module slots are powered on
        self._slots_powered_on = set()

        # Start ssh application

        self._ssh = pexpect.spawn('ssh pi@{}'.format(ip_address2), timeout=50, encoding='utf-8')
        self._ssh.logfile = open("Alabama_pi_log", "w")
        self._ssh.expect("pi@10.116.24.217's password:")
        # time.sleep(3)
        self._ssh.sendline('{}'.format(mypassword))
        self._ssh.expect('0')

        self._ssh.sendline('cd {}'.format(pi_path))
        # child.expect('$')
        self._ssh.expect('0')
        # child.sendline('mkdir atreya')
        self._ssh.sendline("sudo ./pi/{}".format(pi_program))
        # child.expect ('0')
    ...
    def _read_modules(self, delimiter=' '):
        ...
        for __ in range(5):
            ...
            row_values = self._ssh.before.strip().split(delimiter)
            row_values.reverse()
